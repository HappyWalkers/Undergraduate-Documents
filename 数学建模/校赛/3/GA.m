...
%      xy = 10*([x y]-min([x;y]))/(max([x;y])-min([x;y]));
     ...
     close all;
     clc;
     xy=xlsread("C:\Users\mashed potato\Desktop\校赛\附件1.xlsx");
...
%         dmat = reshape(sqrt(sum((xy(a,:)-xy(a',:)).^2,2)),nPoints,nPoints);
    ...
    for iter = 1:numIter
        ...
        for p = 1:popSize
            ...
            pRoute = popRoute(p,:);
            pBreak = popBreak(p,:);
            rng = [[1 pBreak+1];[pBreak n]]';%每行为一个单闭环路线
%             for s = 1:nSalesmen
...
%             end
%             totalDist(p) = d;
totalDist(p)=min_sum_vol(pRoute,rng,dmat,0);%计算电池容量
        end
...
title(hAx,sprintf('最短路径：%1.4f, 代数：%d',minDist,iter));
                hold(hAx,'off');
                drawnow;
            end
        end
...
title(sprintf('最短路径 ： %1.4f',minDist));
            hold on;
        end
