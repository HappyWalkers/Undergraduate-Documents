...
namespace
{
  rcss::RegHolder role = SoccerRole::creators().autoReg(
      &RoleOffensiveHalf::create, RoleOffensiveHalf::NAME);
}
...
RoleOffensiveHalf::doMove(PlayerAgent * agent)
{
  ...
  {
   case Strategy::BA_DribbleBlock://you
    roleoffensivehalfmove().doDefMidMove(agent, home_pos);
    break;
  case Strategy::BA_DefMidField://you
    roleoffensivehalfmove().doDefMidMove(agent, home_pos);
    break;
  ...
  case Strategy::BA_OffMidField:
    roleoffensivehalfmove().doOffensiveMove(agent, home_pos);
    break;
  case Strategy::BA_CrossBlock://you
          roleoffensivehalfmove().doCrossBlockAreaMove( agent, home_pos );  //20140904 need to change
	//rolecenterbackmove().doMarkMove(agent,home_pos);
	//rolesidebackmove().doCrossBlockAreaMove(agent,home_pos);
    break;
  case Strategy::BA_Danger://you
	//rolesidebackmove().doDangerAreaMove(agent,home_pos);
	roleoffensivehalfmove().doDangerAreaMove(agent,home_pos);   //20140904 need to change
	break;
  case Strategy::BA_Cross:
      //when is not same side
    //same side: doOffensiveMove?
