...
SamplePlayer::doShoot()
{
    ...
    if ( wm.gameMode().type() != GameMode::IndFreeKick_
         ...
         && self_pos.x > 22.0 )
    {
        if ( Bhv_StrictCheckShoot().execute( this ) )
        {
            dlog.addText( Logger::TEAM,
                      __FILE__": shooted" );
...
//                      __FILE__": simultaneous kick" );
        ...
        const Vector2D self_pos = wm.self().pos();
        double kick_spped = ServerParam::i().ballSpeedMax();
        Vector2D target_point( 52.5, 0.0 );  //-7.0 -> 52.5

        int receiver = Unum_Unknown;
        double first_speed;
        Vector2D pass_point;
        Vector2D shoot_point1(52.5, -6.24);
        Vector2D shoot_point2(52.5, 6.24);

        Vector2D V0(52.5, 0);

        if ( self_pos.y < 0.0 )
        {
            Vector2D V1(52.5, -7.0);

            //danger :
            if ( (self_pos.x < -46.0 && self_pos.y > -9.0)
                 || ( self_pos.x < -35.0 && wm.self().pos().dist( V0 ) > wm.ball().pos().dist(V0) ) )
            {
...
//                this->setNeckAction(new Neck_ScanField());
                ...
                target_point.y = -20.0;
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>DangerKick(1) ...\n";
                #endif

//                return true;
            }
            //shoot chance (1)
            else if ( self_pos.x > 42.0 && self_pos.y > -9.0)
            {
                target_point = shoot_point1;
                Body_KickOneStep( target_point,
                                    kick_spped
                                    ).execute( this );
                this->setNeckAction( new Neck_TurnToGoalieOrScan() );
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>Shoot(1) ...\n";
                #endif

                return true;
            }
            //side
            else if ( self_pos.x < -49.0 )
            {
                //or to clear? // to kickonestep at present
                target_point = self_pos;
                target_point.x -= 9.0;
            }
            //side->未判断是谁在抢球
            else if ( self_pos.y < -32.0
//                      && wm.ball().vel().x < -0.1
                      && self_pos.x < 20.0 )    //25->20
            {
                target_point = self_pos;
                target_point.y -= 7.0;
            }
            //pass enable
            else if( Body_Pass::get_best_pass( wm, &pass_point, &first_speed, &receiver )
                     && (pass_point.x > -20.0 || pass_point.x > self_pos.x + 4.5)
                     && receiver != wm.self().unum()
                     && receiver != Unum_Unknown
                     && receiver != 1 )  //receiver
            {
                target_point = pass_point;
                kick_spped = first_speed;

                Body_KickOneStep( target_point,
                                    kick_spped
                                    ).execute( this );
                this->setNeckAction( new Neck_TurnToLowConfTeammate() );

                if ( this->config().useCommunication()
                     && receiver != Unum_Unknown )
                {
                    //and say to recerver
                    Vector2D pass_buf = pass_point - self_pos;
                    pass_buf.setLength( 1.0 );    //1.0 or smaller?

                    this->addSayMessage( new PassMessage( receiver,
                                                           pass_point + pass_buf,
                                                           this->effector().queuedNextBallPos(),
                                                           this->effector().queuedNextBallVel() ) );
                }
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>Pass ...\n";
                #endif
            ...
            //not that danger
            else if ( self_pos.x < -35.0 && self_pos.y > -20.0 )
            {
//                Body_ClearBall2009().execute( this );
//                this->setNeckAction(new Neck_ScanField());
                target_point.x = -52.5;
                target_point.y = -17.0;
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>DangerKick(2) ...\n";
                #endif

//                return true;
            }
            //shoot chance (2)
            else if ( self_pos.x > 35.0 && self_pos.y > -12.0 )
            {
                //shoot triangle
//                const rcsc::Triangle2D shoot_triang1 = rcsc::Triangle2D( self_pos, V1, V0);

//                if ( wm.existOpponentIn( shoot_triang1, 2, true ) )
//                {
                    target_point = shoot_point2;    // 0904 21:11
//                }
//                else
//                {
//                    target_point = shoot_point1;
//                }
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>Shoot(2) ...\n";
                #endif
            }
        }
        else
...
//                return true;
            ...
            {
//                const rcsc::Triangle2D shoot_triang2 = rcsc::Triangle2D( self_pos, V2, V0);

//                if ( wm.existOpponentIn( shoot_triang2, 2, true ) )
//                {
                    target_point = shoot_point1;
//                }
//                else
...
//        {
//            std::cerr << world().time().cycle() << __FILE__ << ": Force Cross (org) .?.?.?\n";
////            goal_pos.x = 45.0;
//        }
        Body_KickOneStep( target_point,
                          kick_spped
        ...
        this->setNeckAction( new Neck_ScanField() );
        #ifdef DEBUG2014
        std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>Body_KickOneStep ...\n\n";
        #endif
        return true;
    }
#endif  //1
    ...
    if ( wm.gameMode().type() == GameMode::PlayOn
//         && ! wm.self().goalie()
         && ( wm.self().unum() == 2 || wm.self().unum() == 3 )
         && wm.self().pos().absY() > 7.5
         && wm.self().pos().x < -49.0
         && wm.self().isKickable()
         && nearest_opp_dist < 2.5 )
    {
        ...
        this->setNeckAction( new Neck_ScanField() );
        #ifdef DEBUG2014
        std::cerr << world().time().cycle() << __FILE__ << ": doForceKick>>>Kickout ...\n\n";
        #endif
        return true;
    }
...
*/
bool
SamplePlayer::doSmartTackle()
{
    const WorldModel & wm = this->world();
    const ServerParam & SP = ServerParam::i();

    bool in_danger_area = false;
    ...
    if ( wm.gameMode().type() == GameMode::PlayOn
//         && ! wm.self().goalie()
         && wm.self().tackleProbability() > 0.1
         && !wm.self().isKickable()
         && wm.existKickableOpponent() )
    {
        //side tackle
        if ( ( wm.ball().pos().absY() > 28.0
               || wm.ball().pos().x < -42.0 )
             && wm.self().tackleProbability() > 0.60 )
        {
            if ( wm.ball().pos().absY() < 17.0
                 && wm.self().foulProbability() > 0.8 )
            {
            ...
            else
            {
                ...
                this->doTackle( tackle_dir );
                this->setNeckAction( new Neck_TurnToPoint( ball_next ) );
                #ifdef DEBUG2014
                std::cerr << world().time().cycle() << __FILE__ << ": ###doSmartTackle(1) ...\n";
                #endif
...
#if 1
        if ( //tackle_will_be_in_their_goal
//             || wm.ball().pos().absY() > 28.0
//             || wm.ball().pos().x < -46.0
              wm.self().tackleProbability() < wm.self().foulProbability()
              && wm.self().tackleProbability() > 0.67 ) //ht: magic number
        {
            const TackleGenerator::TackleResult & result2
                    = TackleGenerator::instance().bestResult( wm );

            Vector2D ball_next2 = wm.ball().pos() + result2.ball_vel_;
            double tackle_dir2 = ( result2.tackle_angle_ - wm.self().body() ).degree();

            this->doTackle( tackle_dir2 );
            this->setNeckAction( new Neck_TurnToPoint( ball_next2 ) );
            #ifdef DEBUG2014
            std::cerr << world().time().cycle() << __FILE__ << ": ###doSmartTackle(2) ...\n";
            #endif
    ...
    const Ray2D ball_ray( wm.self().pos(), wm.self().body().degree() );
    const Line2D goal_line_our( Vector2D( -SP.pitchHalfLength(), 10.0 ),
                            Vector2D( -SP.pitchHalfLength(), -10.0 ) );
    const Vector2D intersect = ball_ray.intersection( goal_line_our );

    if ( intersect.isValid()
         && intersect.absY() < SP.goalHalfWidth() + 2.0     //2.0 ok?
         && wm.ball().vel().r() > 0.77
         && wm.self().pos().x < - 36.0
...
//         && ! wm.self().goalie()
         ...
         && !wm.self().isKickable() )
    {
        const TackleGenerator::TackleResult & result3
                    = TackleGenerator::instance().bestResult( wm );

        Vector2D ball_next3 = wm.ball().pos() + result3.ball_vel_;
        double tackle_dir3 = ( result3.tackle_angle_ - wm.self().body() ).degree();

        this->doTackle( tackle_dir3 );
        this->setNeckAction( new Neck_TurnToPoint( ball_next3 ) );
        #ifdef DEBUG2014
        std::cerr << world().time().cycle() << __FILE__ << ": ###doSmartTackle(3) testing...\n";
        #endif
    ...
    //chance tackle
    //
    if ( wm.ball().vel().r() > 2.35
         && wm.self().pos().absX() > 32.0
         && wm.self().tackleProbability() > 0.2
         && wm.self().tackleProbability() < wm.self().foulProbability() )
    {
        const TackleGenerator::TackleResult & result4
                    = TackleGenerator::instance().bestResult( wm );

        Vector2D ball_next4 = wm.ball().pos() + result4.ball_vel_;
        double tackle_dir4 = ( result4.tackle_angle_ - wm.self().body() ).degree();

        this->doTackle( tackle_dir4 );
        this->setNeckAction( new Neck_TurnToPoint( ball_next4 ) );
        #ifdef DEBUG2014
        std::cerr << world().time().cycle() << __FILE__ << ": ###doSmartTackle(4) testing...\n";
        #endif
...
//        double dist_thr;
        ...
        if ( heard_pos.x > wm.theirDefenseLineX()
             && wm.self().pos().x > wm.theirDefenseLineX() - 5.0 )
        {
            //fast attack chance
            dash_power *= 1.5;
        }
        ...
        {
            dash_power *= ( (opp_min - self_min + 2 )*0.1 + 1.0 );

            //ServerParam::i().maxDashPower()=100.0
            dash_power = std::min( dash_power, 150.0);
            dash_power = std::max( dash_power, 85.0);
        }
