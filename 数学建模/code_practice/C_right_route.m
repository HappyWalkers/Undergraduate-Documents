...
%% 先从医院飞上公路
for i=hos_r(1):-1:hos_r(1)-50
    for j=hos_r(2)-10:hos_r(2)+10
        flag=0;
        if a(j,i)==0%最近点
            qi=[i,j];%起点
            hua(1,k)=i;
...
for i=qi(1):-1:1
    for j=qi(2):-1:qi(2)-100
        flag=0;
        if a(j,i)==0%矩阵中遇到黑点
            l_cal=l_cal+1;
            l_dis=dis(hos_r,qi)*lamda_line/lamda+l_cal+dis([i,j],hos_r)*lamda_line/lamda;
            hua(1,k)=i;
            hua(2,k)=j;
            k=k+1;
            if l_dis>=count_fli
...
for i=qi(1)+50:-1:qi(1)-150
    for j=qi(2):qi(2)+160
        flag=0;
        if a(j,i)==0%矩阵中遇到黑点
            r_cal=r_cal+1;
            r_dis=dis(hos_r,qi)*lamda_line/lamda+r_cal+dis([i,j],hos_r)*lamda_line/lamda;
            hua(1,k)=i;
            hua(2,k)=j;
            k=k+1;
            if r_dis>=count_fli
...
text(hos_r(1),hos_r(2),'o','color','r');
