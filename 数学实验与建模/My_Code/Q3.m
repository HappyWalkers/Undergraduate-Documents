...
d2min=18;%第二层厚度范围
d2max=20;
D2_enough=d2min:d2:d2max;%第二层厚度搜索域

d4=0.1;
d4min=6.0;%第四层厚度范围
d4max=6.4;
D4_enough=d4min:d4:d4max;%第四层厚度搜索域

D2_select=100*ones(250,100);%满足条件的第二层厚度值，横坐标表示第二层，纵坐标表示第四层
D4_select=100*ones(250,100);%满足条件的第四层厚度值，横坐标表示第二层，纵坐标表示第四层
for ii=1:length(D2_enough)%对第二层厚度进行搜索，取符合条件得最小值
    for jj=1:length(D4_enough)
        d2_enough=d2min+(ii-1)*d2;%为第二层厚度赋值
        d4_enough=d4min+(jj-1)*d4;%为第四层厚度赋值
        %% 方程3，热扩散方程
        T_sum=1800;%总时间
        d=[0.6;d2_enough;3.6;d4_enough]*10^-3;%厚度
