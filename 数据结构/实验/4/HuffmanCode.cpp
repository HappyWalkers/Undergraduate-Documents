...
int main(){
    // int weight[]={0,7,5,2,4};
    // int n=4;
    int n=0;
    ifstream source;
    source.open("./source.txt",ios::in);//打开要编码的测试文件
    source>>n;
    ...
    char charset[n+1];
    charset[0]=0;
    // cout<<"请输入要编码的字符和其权重"<<endl;
    for(int i=1;i<n+1;i++){
        source>>charset[i]>>weight[i];
        // cin>>charset[i]>>weight[i];
    }
    source.close();
    HuffmanTree ht=CrtHuffmanTree(weight,n,charset);//创建哈夫曼树
    printHuffmanTree(ht,n);

    outfile.open("./hfmTree.txt",ios::out);
    calcHuffmanCode(ht,n);//计算哈夫曼码
    outfile.close();
