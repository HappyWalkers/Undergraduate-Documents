...
void HID_MenuInit(void)
{
  /* Start HID Interface */
	uint16_t line;
	line = (BSP_LCD_GetYSize()/((sFONT *)BSP_LCD_GetFont())->Height);
	
  USBH_UsrLog("Starting HID Demo");
  
  BSP_LCD_SetTextColor(LCD_COLOR_GREEN);
  BSP_LCD_DisplayStringAtLine(line-4, (uint8_t *)"Use [Joystick Left/Right] to scroll up/down");
  BSP_LCD_DisplayStringAtLine(line-3, (uint8_t *)"Use [Joystick Up/Down] to scroll HID menu");
...
void HID_SelectItem(uint8_t **menu, uint8_t item)
{
  ...
  case 0: 
    ...
    BSP_LCD_DisplayStringAtLine(line-2, menu [0]);
    BSP_LCD_SetBackColor(LCD_COLOR_BLUE);    
    BSP_LCD_DisplayStringAtLine(line-1,  menu [1]); 
    break;
