...
void roomPwrManSys_IncmRt(struct ep_info_t *ep, uint16 addr, uint8 endPoint, afMSGCommandFormat_t *msg)
{
    ...
    else
    {
        if(addr == nodeNwkAddr[DevIllum])
        {
            ...
            uint16 i = 0;
            memcpy(&i, msg->Data, 2);
            illumStatus = i < 11500;  //数据11500为判别环境亮暗的参考值，<11500亮，>11500暗，数值11500可以根据串口打印的光照度信息更改。
            HalUARTWrite(HAL_UART_PORT_0, msg->Data, 2);  //串口打印函数打印光照度ADC数据2byte
        }
        else if(addr == nodeNwkAddr[DevIRPers])
        {
            // 接收到人体红外传感器数据
            irPersStatus = !!(msg->Data[0]);
            HalUARTWrite(HAL_UART_PORT_0, &irPersStatus, 1);  //串口打印函数打印人体红外ADC数据1byte
        }
        if(nodeNwkAddr[DevExecuter] != 0xFFFF)
        {
            // 如果执行节点存在
            uint8 ctrl = 0;
            if(irPersStatus && illumStatus) //如果光照度小于设定值并且有人活动，继电器标志置1
                ctrl = 1;
...
void sensorILLumTimeout(struct ep_info_t *ep)
{
    uint16 LightValue = 2047 - (HalAdcRead(0, HAL_ADC_RESOLUTION_14) >> 3);
    // 将光照度变换为AD值的10倍
    LightValue = LightValue * 10;// * 10000 / 256;
