...
{
    const BT4COMM_NODE *node1 = (const BT4COMM_NODE *)n1;
    const BT4COMM_NODE *node2 = (const BT4COMM_NODE *)n2;
#if	0
...
#else
	unsigned char nwkAddr[BT_ADDR_LEN];
	memcpy(nwkAddr,"0",sizeof(nwkAddr));

	if(memcmp(node1->nwkAddr,nwkAddr,BT_ADDR_LEN) == 0)
	    return -1;
	if(memcmp(node2->nwkAddr,nwkAddr,BT_ADDR_LEN) == 0)
	    return 1;

	return memcmp(node1->nwkAddr,node2->nwkAddr,BT_ADDR_LEN);

#endif
}
void sort_node_list(BT4COMM_NODE *nodes, int count)
{
    qsort(nodes, count, sizeof(BT4COMM_NODE), sort_node_list_helper);
}
...
static int find_in_area(BT4COMM_NODE *nodes, int start, int end, unsigned short nwkAddr)
{
    int index;
    for(index = start; index <= end; index++)
    {
         if(memcmp(&nodes[index].nwkAddr, &nwkAddr, BT_ADDR_LEN)==0)
            break;
    }
    if(index > end)
        index = -1;
...
#if		0
int calc_node_level(BT4COMM_NODE *nodes_sorted, int nodeCount, NODE_LEVEL *levels, int maxCount)
{
    int count = 0;
    int index = 1;
...
{
	if(node->nodes)
	    bt4comm_delete_node_list(node->nodes, node->count);
	if(node->node_points)
...
static int nodelist_load(const char *ip, NODELISTHEAD *head, int *maxWidth)
{
    ...
    BT4COMM_NODE *nodes;
    NODE_LEVEL *levels;
    int count = bt4comm_getAllNode(ip, &nodes);
    if(count <= 0)
        return -1;
    ...
    for(i = 0; i < depth; i++)
    {
        ...
        curNode->nodes = bt4comm_node_list_dup(&nodes[levels[i].start], curNode->count);
	if(i == 0)
    ...
    bt4comm_delete_node_list(nodes, count);
    free(levels);
	if(maxWidth != NULL)
		*maxWidth = w;
	return depth;
...
#else
	        while((index + 1 < nodes->count)
	            && (memcmp( nodes->nodes[index + 1].nwkAddr, nodes->nodes[index].nwkAddr, BT_ADDR_LEN) == 0))
#endif
			{
				//// 直接从上一个节点拷贝坐标信息
...
char *utils_bt4addr2str(char *outStr, const unsigned char nwkAddr[BT_ADDR_LEN])
{
    sprintf(outStr, "%02X%02X%02X%02X%02X%02X",
            nwkAddr[0], nwkAddr[1], nwkAddr[2], nwkAddr[3],nwkAddr[4], nwkAddr[5]);
...
//    Json::Value root;
    ...
    while(l != NULL)
    {
		...
		char nwkString[2*BT_ADDR_LEN+1];
            ...
            else
            {
                ...
                if(l->nodes[i].funcNum == 1)
                    strcpy(tip, bt4comm_find_nodeTypeString(l->nodes[i].funcInfo[0].funcCode));
                else if(l->nodes[i].funcNum == 2)
                {
                    sprintf(tip, "%s,%s",
                        bt4comm_find_nodeTypeString(l->nodes[i].funcInfo[0].funcCode),
                        bt4comm_find_nodeTypeString(l->nodes[i].funcInfo[1].funcCode));
                }
                else
                {
                    sprintf(tip, "%s,...", bt4comm_find_nodeTypeString(l->nodes[i].funcInfo[0].funcCode));
                }
            }
            printf(",");
            printf("\"position\":{");
		...
		utils_bt4addr2str(nwkString, l->nodes[i].nwkAddr);
