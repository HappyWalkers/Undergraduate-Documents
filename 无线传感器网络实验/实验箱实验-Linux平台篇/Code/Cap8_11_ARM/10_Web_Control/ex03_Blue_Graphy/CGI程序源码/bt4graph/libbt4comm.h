...
externC EXPORT_SYMBOL BT4COMM_NODE *bt4comm_getNode_byAddr(const char *ip, unsigned char nwkAddr[BT_ADDR_LEN]);
...
externC EXPORT_SYMBOL int bt4comm_sendNode_byPoint(const char *ip, unsigned char nwkAddr[BT_ADDR_LEN], int point, const char *data, int len);
externC EXPORT_SYMBOL int bt4comm_sendNode_byType(const char *ip, int type, int id, const char *data, int len);
externC EXPORT_SYMBOL int bt4comm_getNodeData_byPoint(const char *ip, unsigned char nwkAddr[BT_ADDR_LEN], int point, char **data);
externC EXPORT_SYMBOL int bt4comm_getNodeData_byType(const char *ip, int type, int id, char **data);

//typedef void (*BTCB_NEW_NODE)(void *arg, unsigned char nwkAddr[BT_ADDR_LEN], unsigned short parAddr, unsigned char macAddr[8]);
