 ...
 * @return  none
 */
//不断查询事件表，如果有事件发生就调用相应的事件处理函数;
void osal_run_system( void )
{
  uint8 idx = 0;

#ifndef HAL_BOARD_CC2538
  osalTimeUpdate();//更新系统时钟
#endif
  
  //查看硬件是否有事件发生
  Hal_ProcessPoll();//如果定义了HAL_UART，则初始化UART，一般不define HAL_UART

  do {
    if (tasksEvents[idx])  //得到待处理的最高优先级任务索引号
  ...
  {
    uint16 events;
    halIntState_t intState;//暂存EA

    HAL_ENTER_CRITICAL_SECTION(intState);//进入临界区,保护，关中断
    events = tasksEvents[idx];//提取需要处理的任务中的事件
    tasksEvents[idx] = 0;  // 清除本次任务的事件  Clear the Events for this task.
    HAL_EXIT_CRITICAL_SECTION(intState);//退出临界区,开中断

    activeTaskID = idx;
    events = (tasksArr[idx])( idx, events );//通过指针调用事件处理函数,关键
