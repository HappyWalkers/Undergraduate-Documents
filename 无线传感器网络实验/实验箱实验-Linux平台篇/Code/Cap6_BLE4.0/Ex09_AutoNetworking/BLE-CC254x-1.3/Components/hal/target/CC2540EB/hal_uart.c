...
#else
#if HAL_UART_DMA
  HalUARTPollDMA();         //UART_DMA 20131224 
#endif
#if HAL_UART_ISR
  HalUARTPollISR();
#endif
...
void HalUARTWriteStringValue(char *Input_buf, uint8 newValue,uint8 format)
{
  uint8 tmpLen;
  uint8 buf[HAL_UART_MAX_BUFF];
  uint8 *temp;//[];
  uint32 err;
  tmpLen = (uint8)osal_strlen(buf);
  osal_memcpy(buf,Input_buf,tmpLen);
  //err = (uint32)newValue;
  //temp = _ltoa(err,&buf[tmpLen],format);
  buf[tmpLen] = newValue;
