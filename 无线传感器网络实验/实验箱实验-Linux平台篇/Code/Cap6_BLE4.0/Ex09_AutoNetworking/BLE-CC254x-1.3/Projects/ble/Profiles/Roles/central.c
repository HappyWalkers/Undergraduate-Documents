...
bStatus_t GAPCentralRole_StartDiscovery( uint8 mode, uint8 activeScan, uint8 whiteList )
{
  ...
  params.taskID = gapCentralRoleTaskId;//请求应用的任务ID，用来返回结果;
  params.mode = mode;//扫描模式
  params.activeScan = activeScan;//主动扫描
  params.whiteList = whiteList;//白名单
...
{
  if ( events & SYS_EVENT_MSG )//事件 msg
  {
    uint8 *pMsg;

    if ( (pMsg = osal_msg_receive( gapCentralRoleTaskId )) != NULL )
    {
      gapCentralRole_ProcessOSALMsg( (osal_event_hdr_t *) pMsg );//消息处理函数
